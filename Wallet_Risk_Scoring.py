...
tx_count = df_filtered.groupby('wallet_id').size().rename("total_tx_count")

# Count each type of event per wallet (e.g., how many Borrows, Deposits etc.)
action_counts = df_filtered.groupby(['wallet_id', 'event_name']).size().unstack(fill_value=0)

# Calculate ETH & USD value stats per wallet
volume_stats = df_filtered.groupby('wallet_id').agg({
...
fees_gas_stats = df_filtered.groupby('wallet_id').agg({
...
time_stats = df_filtered.groupby('wallet_id').agg(
...
df_filtered.sort_values(['wallet_id', 'block_signed_at'], inplace=True)
df_filtered['time_diff'] = df_filtered.groupby('wallet_id')['block_signed_at'].diff().dt.total_seconds()
avg_time_gap = df_filtered.groupby('wallet_id')['time_diff'].mean().rename("avg_time_gap")

# Presence of decimals info (if decimals metadata present or not)
decimals_presence = df_filtered.groupby('wallet_id')['sender_contract_decimals'].apply(
...
X = features.drop(columns=['wallet_id', 'first_tx', 'last_tx'])
...
features['score'] = scaler.fit_transform(features[['risk_score']])
...
features_sorted[["wallet_id","score"]].to_csv("Wallet_Risk_Scoring.csv", index=False)


# Plot risk score distribution
plt.figure(figsize=(8, 4))
sns.histplot(features['score'], bins=20, kde=True, color='skyblue', edgecolor='black')
